...
T = 5;            % Simulation time length in real-time units
...
for trial = 1:trial_num
    for n = 1:size(radius_vals, 2)
        % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
        tic
        
        radius = radius_vals(n);
        % Make directory to save images
        folderName = ['forest_radius=' num2str(radius)];
        mkdir(folderName);
        
        % Parameters
        varphi_A = 3;  % Rate of forest seeding into ash
        varphi_G = 3;  % Rate of forest seeding into grass
        ...
        tic;
        f = waitbar(0, 'running FGBA simulation (curvature)');
        while (t < T)
            k = k + 1;
        ...
        % Save figure
        saveas(gcf, [folderName '/cover_prop_trial=' num2str(trial) '.png']);
        saveas(gcf, [folderName '/cover_prop_trial=' num2str(trial) '.svg']);
