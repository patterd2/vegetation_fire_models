...
varphi_vals = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4];


%% Begin loop
for trial = 1:num_trials
    for n = 1:size(varphi_vals, 2)
        % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
        tic
    
        % Make directory to save images
        folderName = ['varphi=' num2str(varphi_vals(n))];
        mkdir(folderName);
        
        % Parameters
        varphi_A = varphi_vals(n);  % Rate of forest seeding into ash
        varphi_G = varphi_vals(n);  % Rate of forest seeding into grass
        ...
        % Save figure
        saveas(gcf, [folderName '/cover_prop_trial_' num2str(trial) '.png']);
        saveas(gcf, [folderName '/cover_prop_trial_' num2str(trial) '.svg']);
