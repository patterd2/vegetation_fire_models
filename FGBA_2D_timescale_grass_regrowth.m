...
T = 0.02;            % Simulation time length in real-time units
...
for n = 1:size(param_nums, 2)
    ...
    folderName = 'timescale_grass_regrowth_v2';
    mkdir(folderName);
    ...
    sigma_B = 0.04;    % width of Gaussian for burning spread over grassland
    ...
    if (make_video)
        ...
        for i = 1:length(0:dt:T)
            ...
            figure(2);
            set(figure(2), 'position', [0, 100, 500, 500])
            im = imagesc(Sol_Save(:,:,i));
            daspect([1 1 0.2])
            title(['t = ' num2str(dt * i)], 'Interpreter', 'latex')
            xlim([1, L / dx])
            ylim([1, L / dx])
            set(gca, 'xtick', [])
            set(gca, 'ytick', [])
            ...
            saveas(figure(2), [folderName '/t=' num2str(dt*i, '%2.6f') '.png']);
            saveas(figure(2), [folderName '/t=' num2str(dt*i, '%2.6f') '.svg']);
            %imwrite(imdata.cdata, [folderName '/t=' num2str(dt*i, '%2.6f') '.png']);
            %imwrite(imdata.cdata, [folderName '/t=' num2str(dt*i, '%2.6f') '.svg']);
