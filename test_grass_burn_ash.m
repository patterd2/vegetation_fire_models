...
for p = 1:size(prop_ash, 2)
    for b = 1:size(beta_G_vals, 2)
        for n = 1:num_trials
            ...
            fprintf(['prop_ash=' num2str(prop_ash(p)) '  beta_G=' num2str(beta_G_vals(b)) '  trial_' num2str(n)])
...
[X, Y] = meshgrid(prop_ash, beta_G_values);
scatter(X, Y, avg_burn_times)
errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)

legend('expected', 'measured')

saveas(gcf, 'testing_figs/grass_fire_spread.jpeg');
