...
% Parameters
L = 1;            % Working on [0, L]
num_sites = 30^2; % Total number of sites in [0, L] x [0, L], must be square number
...
is_G = false(num_sites, 1);
is_B = false(num_sites, 1);
is_F = false(num_sites, 1);
is_A = false(num_sites, 1);
...
while (t < T)
    k = k + 1;
    % Integrate W_G gaussian over G sites only
    GG_integral = G_integral;
    GG_integral(:, not(is_G)) = 0;
    sum_GG_integral = sum(GG_integral, 2);

    % Integrate W_B gaussian over B sites only
    BB_integral = B_integral;
    BB_integral(:, not(is_B)) = 0;
    sum_BB_integral = sum(BB_integral, 2);

    % Integrate W_F gaussian over F sites only
    FF_integral = F_integral;
    FF_integral(:, not(is_F)) = 0;
    sum_FF_integral = sum(FF_integral, 2);

    % Calculate event rate at each site
    eventRatePerSite = is_G .* (varphi_G * sum_FF_integral + phi_G(sum_GG_integral) + beta_G * sum_BB_integral) ...
        + is_F .* (mu + phi_F(sum_GG_integral) + beta_F * sum_BB_integral) ...
        + is_B .* q + ...
        + is_A .* (gamma + varphi_A * sum_FF_integral);
    ...
    if (t<T) % Event occurs before end of simulation
        ...
        if (X(site, k) == 0)
            GB_rate = phi_G(sum(GG_integral(site, :))) + beta_G * sum(BB_integral(site, :));
