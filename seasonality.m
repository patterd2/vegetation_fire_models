...
% Variant allowing for 2 seasons
clear
close all
...
for n = 1:1
    ...
    season_1_saved = false;
    season_2_saved = false;
    ...
    tic;
    f = waitbar(0, 'running FGBA simulation (seasonality)');
    while (t < T)
        if (season_changed == false) % Do not increment time index for season change
            k = k + 1;
        end
            
        % Season Parameters
        if (mod(season_num, 2) == 1)
            ...
            f1 = 0.02;         % Forest burning sigmoid upper bound
            s_F = 0.05;       %orest burning sigmoid width
        
            theta_G = 0.6;    % Grass burning sigmoid center
            g0 = 0.05;        % Grass burning sigmoid lower bound
            g1 = 0.05;        % Grass burning sigmoid upper bound
            s_G = 0.05;       % Grass burning sigmoid width
    
            % Save all parameters first time
            if (season_2_saved == false)
                save([folderName '/season_2_parameters.mat'], 'L', 'num_sites', 'T', 't0', ...
                    'varphi_G', 'varphi_A', 'beta_G', 'beta_F', 'gamma', 'q', 'mu', ...
                    'sigma_P', 'sigma_B', 'sigma_G' ,'sigma_F', 'theta_F', 'f0', 'f1', ...
                    's_F', 'theta_G', 'g0', 'g1', 's_G')
                season_2_saved = true; 
        ...
        if ((k == 1) || (season_changed)) % Recompute integrals first time or after season change
            % Gaussian functions
            ...
            g = waitbar(0, ['computing integrals for season ' num2str(season_num)]);
            for i = 1:num_sites
                for j = 1:num_sites
        ...
        if (t + NextEvent < season_num * season_length) % Event occurs before end of season
            t = t + NextEvent;
        ...
        else % Event occurs after end of season
            if (t + NextEvent > T)
                Times(end + 1) = T;
                X(:, k + 1) = X(:, k);
            else
