...
num_trials = 5;
mu_vals = [2e-3, 4e-3, 6e-3, 8e-3, 1e-2, 1.2e-2, 1.4e-2, 1.6e-2, 1.8e-2, 2e-2];
death_times = zeros(num_sites, size(mu_vals, 2), num_trials);
...
for muu = 1:size(mu_vals, 2)
    for n = 1:num_trials
        % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
        tic
    
        mu = mu_vals(muu);         % Fire quenching rate
        fprintf(['mu=' num2str(mu_vals(muu)) ' trial ' num2str(n) '\n'])
        ...
        %% Initialize states of sites (all forest)
        for i = 1:num_sites
            X(i, 1) = 1;
        end
...
trials_avg_death = mean(death_times, 3);
trials_space_avg_death = mean(trials_avg_death, 1);
trials_space_var_death = var(death_times, 0, [1, 3]);

death_x_axis = linspace(mu_vals(1), mu_vals(end), 1000);
expected_death = 1 ./ death_x_axis;

plot(death_x_axis, expected_death, 'r');
hold on
scatter(mu_vals, trials_space_avg_death, 'b', 'filled')
errorbar(mu_vals, trials_space_avg_death, sqrt(trials_space_var_death) / 2, 'LineStyle', 'none', 'Color', 'b', 'LineWidth', 2);
xlabel('Tree mortality rate (yr$$^{-1}$$)', 'Interpreter','latex')
ylabel('Tree lifespan (yr)', 'Interpreter','latex')
legend('expected', 'measured')

set(figure(1),'position',[0, 100, 500, 500])
saveas(gcf, 'forest_mortality.svg');
saveas(gcf, 'forest_mortality.png');
saveas(gcf, 'forest_mortality.jpeg');
