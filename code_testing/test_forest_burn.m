%% Measure time for forest patch surrounded by fire and ash to burn
clear
close all
...
%prop_ash = 0.1;
beta_F_vals = [2e3, 4e3, 6e3, 8e3, 1e4, 1.2e4, 1.4e4, 1.6e4, 1.8e4];
%beta_F_vals = 1e4;
num_trials = 600;
burn_times = zeros(size(prop_ash, 2), size(beta_F_vals, 2), num_trials);
...
for p = 1:size(prop_ash, 2)
    for b = 1:size(beta_F_vals, 2)
        fprintf(['prop_ash=' num2str(prop_ash(p)) '  beta_F=' num2str(beta_F_vals(b)) '\n'])
        for n = 1:num_trials
            ...
            beta_F = beta_F_vals(b);    % Rate of fire spread through forest
            ...
            forest_site = ceil(rand() * num_sites);
            ...
            while (t < T)
                ...
                %fprintf(['intensity: ' num2str(totalIntensity)]);
...
save('burn_times_forest.mat', 'burn_times')
colors = get(gca, 'ColorOrder');

avg_burn_times = mean(burn_times, 3);
var_burn_times = var(burn_times, 0, 3);
for i = 1:size(prop_ash, 2)
    plot(beta_F_vals, avg_burn_times(i, :), '.', 'MarkerEdgeColor',colors(i,:), 'LineStyle','none')
    %plot(beta_F_vals, avg_burn_times(i, :))
    hold on
end
%% Plot expected
X = linspace(min(beta_F_vals), max(beta_F_vals));

for i = 1:size(prop_ash, 2)
    plot(X, 1 ./ ((1-prop_ash(i)) * X), 'Color', colors(i,:))
end
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)
xlabel('$\beta_F$', 'Interpreter', 'latex')
ylabel('Burn time (yrs)', 'Interpreter', 'latex')
leg2 = legend('$p_{ash}=0.10$', '$p_{ash}=0.25$', '$p_{ash}=0.40$', '$p_{ash}=0.55$', '$p_{ash}=0.70$', '$p_{ash}=0.85$', 'Interpreter', 'latex');

set(figure(1),'position',[0, 100, 500, 500])
saveas(gcf, 'burn_forest.svg');
saveas(gcf, 'burn_forest.png');
saveas(gcf, 'burn_forest.jpeg');

toc

% Plot surface
%[X,Y] = meshgrid(0.1:0.01:0.9, beta_F_vals(1):10:beta_F_vals(end));
%Z = 1 ./ ((1 - X) .* Y);
%s = surf(X,Y,Z,'EdgeColor', 'none');
%xlabel('Proportion grass', 'Interpreter','latex')
%ylabel('$$\beta_F$$ (yr$^{-1}$)' , 'Interpreter', 'latex')
%zlabel('Burn time (yr)', 'Interpreter','latex')
...
%avg_burn_times = mean(burn_times, 3);
%var_burn_times = var(burn_times, 0, 3);
%[X, Y] = meshgrid(flip(prop_ash), flip(beta_F_vals));
%scatter3(X, Y, avg_burn_times, 'k', 'filled')
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)

%saveas(gcf, 'testing_figs/forest_fire_spread.jpeg');
