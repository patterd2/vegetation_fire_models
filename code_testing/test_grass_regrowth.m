...
num_trials = 5;
gamma_vals = [2e1, 4e1, 6e1, 8e1, 1e2, 1.2e2, 1.4e2, 1.6e2, 1.8e2, 2e2];
regrowth_times = zeros(num_sites, size(gamma_vals, 2), num_trials);
...
for gg = 1:size(gamma_vals, 2)
    for n = 1:num_trials
        % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
        tic
    
        gamma = gamma_vals(gg);         % Fire quenching rate
        fprintf(['gamma=' num2str(gg) ' trial ' num2str(n) '\n'])
        ...
        %% Initialize states of sites (all ash)
        for i = 1:num_sites
            X(i, 1) = 3;
        end
...
trials_avg_regrowth = mean(regrowth_times, 3);
trials_space_avg_regrowth = mean(trials_avg_regrowth, 1);
trials_space_var_regrowth = var(regrowth_times, 0, [1, 3]);

regrowth_x_axis = linspace(gamma_vals(1), gamma_vals(end), 1000);
expected_regrowth = 1 ./ regrowth_x_axis;

plot(regrowth_x_axis, expected_regrowth, 'r');
hold on
scatter(gamma_vals, trials_space_avg_regrowth, 'b', 'filled')
errorbar(gamma_vals, trials_space_avg_regrowth, sqrt(trials_space_var_regrowth) / 2, 'LineStyle', 'none', 'Color', 'b', 'LineWidth', 2);
xlabel('Grass regrowth rate (yr$$^{-1}$$)', 'Interpreter','latex')
ylabel('Grass regrowth time (yr)', 'Interpreter','latex')
legend('expected', 'measured')

saveas(gcf, 'grass_regrowth.jpeg');

set(figure(1),'position',[0, 100, 500, 500])
saveas(gcf, 'grass_regrowth.svg');
saveas(gcf, 'grass_regrowth.png');
saveas(gcf, 'grass_regrowth.jpeg');
