...
fig_num = 1;
prop_grass = [0.1, 0.25, 0.4, 0.55, 0.7, 0.85];
varphi_A_vals = [2e-2, 4e-2, 6e-2, 8e-2, 1e-1, 1.2e-1, 1.4e-1, 1.6e-1, 1.8e-1];
num_trials = 600;
growth_times = zeros(size(prop_grass, 2), size(varphi_A_vals, 2), num_trials);

tic
%% Begin loop
for p = 1:size(prop_grass, 2)
    for vA = 1:size(varphi_A_vals, 2)
        fprintf(['prop_grass=' num2str(prop_grass(p)) '  varphi_A=' num2str(varphi_A_vals(vA)) '\n'])
...
save('growth_times_forest_ash.mat', 'growth_times')
colors = get(gca, 'ColorOrder');

avg_growth_times = mean(growth_times, 3);
var_growth_times = var(growth_times, 0, 3);
for i = 1:size(prop_grass, 2)
    plot(varphi_A_vals, avg_growth_times(i, :), '.', 'MarkerEdgeColor', colors(i,:), 'LineStyle', 'none')
    hold on
end

%% Plot expected
X = linspace(min(varphi_A_vals),max(varphi_A_vals));

for i = 1:size(prop_grass, 2)
    plot(X, 1 ./ ((1-prop_grass(i)) * X), 'Color', colors(i,:))
end
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)
xlabel('$\varphi_A$', 'Interpreter', 'latex')
...
saveas(gcf, 'forest_ash_spread.svg');
saveas(gcf, 'forest_ash_spread.png');
saveas(gcf, 'forest_ash_spread.jpeg');
