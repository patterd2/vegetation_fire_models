...
save('tree_ash_growth_times.mat', 'growth_times')

% Plot surface
[X,Y] = meshgrid(0.1:0.01:0.9, varphi_A_vals(1):0.001:varphi_A_vals(end));
Z = 1 ./ ((X) .* Y);
s = surf(X,Y,Z,'EdgeColor', 'none');
xlabel('Proportion forest', 'Interpreter','latex')
ylabel('$$\varphi_A$$ (yr$^{-1}$)' , 'Interpreter', 'latex')
...
[X, Y] = meshgrid(prop_grass, flip(varphi_A_vals));
scatter3(X, Y, avg_growth_times, 'k', 'filled')
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)

saveas(gcf, 'testing_figs/forest_ash_spread.jpeg');
