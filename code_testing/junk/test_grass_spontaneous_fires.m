%% Measure spontaneous grass fire rate as function of forest/grass ratio
clear
close all
...
%prop_grass = 0.9;
g0_vals = [0.05, 0.15, 0.25, 0.35, 0.45] / num_sites;
g1_vals = [0.95, 0.85, 0.75, 0.65, 0.55] / num_sites;
num_trials = 5;
grass_fires_rate = zeros(size(prop_grass, 2), size(g0_vals, 2), num_trials);


%% Begin loop
for p = 1:size(prop_grass, 2)
    for g00 = 1:size(g0_vals, 2)
        for n = 1:num_trials
            % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
            tic

            fprintf(['prop_grass=' num2str(prop_grass(p)) '  g0=' num2str(g0_vals(g00)) ' g1=' num2str(g1_vals(g00)) '  trial ' num2str(n) '\n'])
                ...
                if (t<T) % Event occurs before end of simulation
                    ...
                    if (X(site, k) == 0)
                        ...
                        if (rand() < prob_GB)
                            ...
                            % occur
                            grass_fire_count = grass_fire_count + 1;
                            grass_fires_rate(p, g00, n) = 1 / grass_fire_count;
...
ylabel('$$2g_0 = 2g_1 - 1$$', 'Interpreter', 'latex')
zlabel('Spontaneous fire rate ($N^{-1}$ yr$$^{-1}$$)', 'Interpreter','latex')

hold on

% Plot data
avg_grass_fires = mean(grass_fires_rate, 3);
var_grass_fires = var(grass_fires_rate, 0, 3);
[X, Y] = meshgrid(prop_grass, (2 * num_sites) .* g0_vals );
scatter3(X', Y', 10 .* avg_grass_fires)
%errorbars(X, Y, avg_grass_fires, sqrt(var_grass_fires)/2)

legend('expected', 'measured')

saveas(gcf, 'testing_figs/grass_spontaneous_fires.jpeg');
