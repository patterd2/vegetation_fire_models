...
T = 20;            % Simulation time length in real-time units
...
fig_num = 1;
prop_ash = [0.1, 0.25, 0.4, 0.55, 0.70, 0.85];
%prop_ash = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9];
...
for p = 1:size(prop_ash, 2)
    for b = 1:size(beta_G_vals, 2)
        fprintf(['prop_ash=' num2str(prop_ash(p)) '  beta_G=' num2str(beta_G_vals(b)) '\n'])
        for n = 1:num_trials
...
save('burn_times_grass.mat', 'burn_times')
colors = get(gca, 'ColorOrder');

avg_burn_times = mean(burn_times, 3);
var_burn_times = var(burn_times, 0, 3);
for i = 1:size(prop_ash, 2)
   plot(beta_G_vals, avg_burn_times(i, :), '.', 'MarkerEdgeColor', colors(i,:), 'LineStyle', 'none')
    %plot(beta_G_vals, avg_burn_times(i, :))
    hold on
end

%% Plot expected
X = min(beta_G_vals):1000:max(beta_G_vals);

for i = 1:size(prop_ash, 2)
    plot(X, 1 ./ ((1-prop_ash(i)) * X), 'Color', colors(i,:))
end
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)
xlabel('$\beta_G$', 'Interpreter', 'latex')
ylabel('Burn time (yrs)', 'Interpreter', 'latex')
legend('$p_{ash}=0.10$', '$p_{ash}=0.25$', '$p_{ash}=0.40$', '$p_{ash}=0.55$', '$p_{ash}=0.70$', '$p_{ash}=0.85$', 'Interpreter', 'latex')

set(figure(1),'position',[0, 100, 500, 500])
saveas(gcf, 'burn_grass.svg');
saveas(gcf, 'burn_grass.png');
saveas(gcf, 'burn_grass.jpeg');
