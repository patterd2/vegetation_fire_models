%% Measure time for grass patch surrounded by forest/ash to grow tree
clear
close all
...
fig_num = 1;
prop_ash = [0.1, 0.25, 0.4, 0.55, 0.7, 0.85];
varphi_G_vals = [2e-2, 4e-2, 6e-2, 8e-2, 1e-1, 1.2e-1, 1.4e-1, 1.6e-1, 1.8e-1];
num_trials = 600;
growth_times = zeros(size(prop_ash, 2), size(varphi_G_vals, 2), num_trials);

tic

%% Begin loop
for p = 1:size(prop_ash, 2)
    for vG = 1:size(varphi_G_vals, 2)
        fprintf(['prop_ash=' num2str(prop_ash(p)) '  varphi_G=' num2str(varphi_G_vals(vG)) '\n'])

        for n = 1:num_trials
            % Convention for state labels: 0 = Grass, 1 = Forest, 2 = Burning, 3 = Ash
            
            % Parameters
            varphi_A = 0;  % Rate of forest seeding into ash
            varphi_G = varphi_G_vals(vG);  % Rate of forest seeding into grass
            ...
            sigma_P = 0.2;   % width of Gaussian of forest seeding
            sigma_B = 0.2;    % width of Gaussian for burning spread over grassland
            sigma_G = 0.2;    % width of Gaussian for flammability of large grass
            sigma_F = 0.2;    % width of Gaussian for flammability of large grass
            ...
            %% Initialize states of sites (one grass site + mixture of forest/ash)
                    ...
                    if (X(site, k) == 0)
                        ...
                        if (rand() < prob_GB)
                            ...
                            if (time_recorded == false)
                                growth_times(p, vG, n) = t;
                                time_recorded = true;
                                t = T + 1; % to end simulation
                            end
            ...
            % time the calculations of the Gillespie algorithm
        end
    end
end
...
save('growth_times_forest_grass.mat', 'growth_times')
colors = get(gca, 'ColorOrder');

avg_growth_times = mean(growth_times, 3);
var_growth_times = var(growth_times, 0, 3);
for i = 1:size(prop_ash, 2)
    plot(varphi_G_vals, avg_growth_times(i, :), '.', 'MarkerEdgeColor', colors(i,:), 'LineStyle','none')
    %plot(varphi_G_vals, avg_burn_times(i, :))
    hold on
end

%% Plot expected
X = linspace(min(varphi_G_vals), max(varphi_G_vals));

for i = 1:size(prop_ash, 2)
    plot(X, 1 ./ ((1-prop_ash(i)) * X), 'Color', colors(i,:))
end
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)
xlabel('$\varphi_G$', 'Interpreter', 'latex')
ylabel('Growth time (yrs)', 'Interpreter', 'latex')
legend('$p_{forest}=0.90$', '$p_{forest}=0.75$', '$p_{forest}=0.60$', '$p_{forest}=0.45$', '$p_{forest}=0.30$', '$p_{forest}=0.15$', 'Interpreter', 'latex')

set(figure(1),'position',[0, 100, 500, 500])
saveas(gcf, 'forest_grass_spread.svg');
saveas(gcf, 'forest_grass_spread.png');
saveas(gcf, 'forest_grass_spread.jpeg');
...
% Plot surface
%[X,Y] = meshgrid(0.1:0.01:0.9, varphi_G_vals(1):0.001:varphi_G_vals(end));
%Z = 1 ./ (X .* Y);
%s = surf(X,Y,Z,'EdgeColor', 'none');
%xlabel('Proportion forest', 'Interpreter','latex')
%ylabel('$$\varphi_G$$ (yr$^{-1}$)' , 'Interpreter', 'latex')
%zlabel('Tree growth (yr)', 'Interpreter','latex')
...
%avg_growth_times = mean(growth_times, 3);
%var_growth_times = var(growth_times, 0, 3);
%[X, Y] = meshgrid(prop_ash, flip(varphi_G_vals));
%scatter3(X, Y, avg_growth_times, 'k', 'filled')
%errorbars(X, Y, avg_burn_times, sqrt(var_burn_times)/2)

%saveas(gcf, 'testing_figs/forest_grass_spread.jpeg');
