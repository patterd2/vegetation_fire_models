...
for i = 1:num_sites
    for j = 1:num_sites
        ...
        % X(i, j) = (G, F)
        if (X(i, 1) == 0) && (X(j, 1) == 1)
            rate_matrix(i, j) = alpha_G * W_F(Locations(i), Locations(j));
        end

        % X(i, j) = (A, F)
        if (X(i, 1) == 3) && (X(j, 1) == 1)
            rate_matrix(i, j) = alpha_A * W_F(Locations(i), Locations(j));
        end
        ...
        % X(i, j) = (G, B)
        if (X(i, 1) == 0) && (X(j, 1) == 1)
            rate_matrix(i, j) = beta_G * W_B(Locations(i), Locations(j));
        end
    end
end
...
while (t < T)
    ...
    for i = 1:num_sites
        ...
        if (X(site, k + 1) == 0) && (X(i, k + 1) == 1)
            rate_matrix(site, i) = alpha_G * W_F(Locations(site), Locations(i));
        end

        if (X(site, k + 1) == 3) && (X(site, k + 1) == 1)
            rate_matrix(site, i) = alpha_A * W_F(Locations(site), Locations(i));
        end
